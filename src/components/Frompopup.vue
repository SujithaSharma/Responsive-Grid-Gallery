...
<Card>
    <template v-slot:slot="slotProps">
...
<br><br>
<button @click="active = 'DA'">DynamicA</button>
<button @click="active = 'DB'">DynamicB</button>
<button @click="active = 'DC'">DynamicC</button>
<!-- <component :is="active" /> -->
 <DynamicA v-if="active==='DA'"></DynamicA>
<DynamicB v-if="active==='DB'" ></DynamicB>
<DynamicC v-if="active==='DC'"></DynamicC> 
</template>

<script>
import Popup from './Showpopup.vue'
...
export default {
    ...
    methods:{
        close(name) {
            ...
            console.log('Console is printed with name',name)
        },
    }
}
</script>
